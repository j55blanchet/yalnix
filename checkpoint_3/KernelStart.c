...
void KernelStart(char** cmd_args, unsigned int pmem_size, UserContext* u_context)
{
  ...
  {
    init_pcb->pid    = NEXT_PID;
    init_pcb->ppid   = -1; // No parent.
    init_pcb->status = 0;
    init_pcb->ticks  = 0;
    init_pcb->r1_stack_base_index  = r1_addr_to_id(VMEM_1_LIMIT);
    init_pcb->r1_break_limit_index = r1_addr_to_id(VMEM_1_BASE);
  ...
  // Idle process.
  {
    ...
    if (!init_is_started){
      ...
      // Load program.
      int rv = LoadProgram(cmd_args[0], cmd_args, init_pcb);
	...
	Halt();
      ...
      // JHL. Why does changing address space necessary here?
      ...
      rv = LoadProgram("IdleProcess", args, idle_pcb);
      // JHL. What do we do if LoadProgram() fails?
      // It shouldn't but...
      if( rv == ERROR ) {
