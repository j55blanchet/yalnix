...
void KernelStart(char** cmd_args, unsigned int pmem_size, UserContext* u_context) {
  ...
  assert(pcb_array);
  interp_array    = (interp_t**)calloc(sizeof(interp_t*), interp_array_size);
  assert(interp_array);
  ...
  InitQueue(&SLEEPING);
  InitQueue(&WAITING);
  WRITING = (queue_t*)calloc(NUM_TERMINALS, sizeof(queue_t));
  assert(WRITING);
  READING = (queue_t*)calloc(NUM_TERMINALS, sizeof(queue_t));
  assert(READING);
  WRITING_WAIT = (queue_t*)calloc(NUM_TERMINALS, sizeof(queue_t));
  assert(WRITING_WAIT);
  { int i;
    for(i = 0; i < NUM_TERMINALS; i++) {
      InitQueue(&WRITING[i]);
      InitQueue(&READING[i]);
      InitQueue(&WRITING_WAIT[i]);
    }
  }

  // Initialize all terminal buffers.
  tty_write_buffer = (char**)calloc(NUM_TERMINALS, sizeof(char*));
  assert(tty_write_buffer);
  tty_read_buffer  = (char**)calloc(NUM_TERMINALS, sizeof(char*));
  assert(tty_read_buffer);
  { int i;
    for(i = 0; i < NUM_TERMINALS; i++) {
      tty_write_buffer[i] = (char*)calloc(TERMINAL_MAX_LINE, sizeof(char));
      assert(tty_write_buffer[i]);
      tty_read_buffer[i]  = (char*)calloc(TERMINAL_MAX_LINE, sizeof(char));
      assert(tty_read_buffer[i]);
    }
  }
