...
pcb_t* InitPCB(int ppid) {
  ...
  pcb->r1_stack_base_index  = r1_addr_to_id(VMEM_1_LIMIT);
  pcb->r1_break_limit_index = r1_addr_to_id(VMEM_1_BASE);
...
soul_t* KillPCB(pcb_t* pcb, int status) {
  ...
  if( pcb->prev != NULL || pcb->next != NULL ) {
    TracePrintf(TRACE_WRONG, "KillPCB(): process #%d is still in queue.\n", pcb->pid);
    Halt();
  }
